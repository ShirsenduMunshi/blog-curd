...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchBlogCount() {
      try {
        const response = await axios.get("/api/blog");
        setBlogCount(response.data.total || 0);
      } catch (error) {
        toast.error("Failed to fetch blog count.");
        console.error("Failed to fetch blog count:", error);
      }
    }
    fetchBlogCount();
  ...
  useEffect(() => {
    ...
    async function fetchUserBlogs() {
      if (!loggedInUser) return; // Ensure user is logged in
      try {
        setLoading(true);
        const response = await axios.get(`/api/blog/?author=${loggedInUser.name}`);
